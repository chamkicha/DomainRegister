...
class MyfredEPP {
	...
	*/
	public function __construct($connect = true) {
		...
		if($connect && !$this->socket) {
            $FredClients = DB:: table('FredClients')->first();
			$EPP_HOST = $FredClients->host;
			$EPP_PORT = $FredClients->port;
			$EPP_TIMEOUT = 60;
	...
	*/
	function Connect($host, $port = 700, $timeout = 60) {
        ...
        $port = 700;
        $path = '/var/www/html/register/keys/';
        $transport = 'tls';
        $ssl = ['ssl' => [
                  'local_cert'  => $path . 'certs.pem',       // SSL Certificate
		...
		// $fileContent = file_get_contents($path . 'certs.pem');
		// dd($fileContent);
        $ssl_context = stream_context_create($ssl);
        $this->socket = stream_socket_client($transport . '://' . $host . ':' . $port, $errno, $errstr,5, STREAM_CLIENT_CONNECT, $ssl_context);

		if (!$this->socket) 
		{  
		    $die = $errstr.",".$errno.",".file_get_contents($path);
	     	//die($die);
...
, FILE_APPEND);
	...
	* Functions below are set to public
	...
	*/
	...
	/**
	* public function used for binary logging of all frames.
