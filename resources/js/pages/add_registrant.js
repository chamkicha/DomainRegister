...
$('#add_registrant').bootstrapValidator({
    fields: {
        first_name: {
            validators: {
                notEmpty: {
...
$('#rootwizard .finish').click(function() {
    ...
    if ($validator.isValid()) {
        document.getElementById('add_registrant').submit();
    }
});
$('#activate').on('ifChanged', function(event) {
    $('#add_registrant').bootstrapValidator('revalidateField', $('#activate'));
});
$('#add_registrant').keypress(function(event) {
    if (event.which == '13') {
